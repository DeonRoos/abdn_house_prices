...
url <- "https://api.aspc.co.uk/Property/GetProperties?PrimaryPropertyType=Rent&SortBy=PublishedDesc&LastUpdated=AddedAnytime&PropertyType=Residential&HasGarage=false&HasDoubleGarage=false&HasGarden=false&IsNewBuild=false&HasGreenEnergy=false&HasOnStreetParking=false&HasOffStreetParking=false&HasElectricVehicleChargePoint=false&HasGrannyFlat=false&IsSelfContained=false&HasGroundFloorBedroomBathroom=false&IsLowCostHome=false&IsHmoLicense=false&IsAllowPets=false&IsAllowSmoking=false&IsFullyFurnished=false&IsPartFurnished=false&IsUnfurnished=false&ExcludeUnderOffer=false&IncludeClosedProperties=true&ClosedDatesSearch=14&ResultMode=NONE&ResultView=LIST&search-origin=search-results&Sort=PublishedDesc&Page=1&PageSize=12"
...
if (dbExistsTable(db, table_name)) {
  message("Reading existing rent data from SQLite table: ", table_name)
  existing_data <- dbReadTable(db, table_name)
} else if (file.exists(file_path)) {
  message("No rent_data table found in DB; reading from CSV instead.")
...
if (nrow(new_properties_df) > 0) {
  new_properties_df <- new_properties_df %>%
    ...
    mutate(
      ...
      property_url    = paste0(
        "https://www.aspc.co.uk/search/property/", 
        Id, "/", AddressLineDash, "/", City, "/"
      )
    )
